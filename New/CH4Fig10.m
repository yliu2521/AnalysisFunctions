...
legend('1-item','4-item','6-item') % ('1-item','2-item','3-item','4-item','5-item','6-item','7-item') %
...
for NumP = 1:7
    SpikeMat = cell(repeat,1);
        for id_out = 1:repeat
            ...
            if isempty(R.ExplVar)
                SpikeMat{id_out} = SpikeMat{id_out-1};
                continue
            end
            SpikeMat{id_out} = mean(R.spike_hist{1});
        end
    fr = movsum(mean(cell2mat(SpikeMat)),span);
    plot(0.1*(1:1e5),fr,'color',Color(NumP,:))
    hold on
end
xlabel('Time (ms)','fontSize',10)
ylabel('Network firing rate','fontSize',10)
...
for NumP = 1:7
    ...
    bin = 1e3:1e3:2e4;
    SpikeMat = zeros(repeat,length(bin));
    for i = 1:length(bin)
        for id_out = 1:repeat
            fprintf('Processing output file No.%d out of %d...\n', id_out+repeat*(NumP-1), num_files);
            fprintf('\t File name: %s\n', files{id_out+repeat*(NumP-1)});
            R = load(files{id_out+repeat*(NumP-1)});
            if isempty(R.ExplVar)
                SpikeMat(id_out,i) = SpikeMat(id_out-1,i);
                continue
            end
            for no = 1 % :NumP
